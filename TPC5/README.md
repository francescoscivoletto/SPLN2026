...
a Transformer-based classroom model from aula9.ipynb, based on BERT
a spaCy NER model trained on the provided .iob files
...
convert the .iob files into .spacy format
initialize a spaCy configuration file for Portuguese NER
train the model using the training split
evaluate the best saved model on the validation split
compare the results with the classroom model
Files used
